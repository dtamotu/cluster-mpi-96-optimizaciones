...
\item \textbf{La diferencia está en el cálculo, no en la red.} La tabla~\ref{tab:variabilidad-fases} separa las fases de las configuraciones con 24 procesos.
\end{itemize}
...
\input{tablas/variabilidad_fases}
\end{table}

$T_{Dist}$ cambia poco (hasta un 11\,\%), mientras $T_{Calc}$ crece entre 1.3 y 1.5 veces en la campaña (0.304 a 0.455 s con $N=3072$ por Ethernet; 2.891 a 4.447 s con $N=6144$). Esto es coherente con la diferencia de compilación: sin \texttt{-march=native}, el compilador solo puede vectorizar con el conjunto básico x86-64 (SSE2) y no emite las instrucciones AVX2 y FMA del procesador. No es una prueba aislada, porque también cambió el momento de ejecución; comprobarlo requiere recompilar con los mismos flags y repetir.
...
\begin{enumerate}
\item Las conclusiones cualitativas se mantienen: en todas las fuentes, un nodo con 24 procesos es el mejor punto para matrices y la difusión entre nodos domina el tiempo con 96 procesos.
\item Los cocientes $T_{96}/T_{24}$ de la campaña (4.77 y 4.57) no deben mezclarse con los de las baterías (6.52 para $N=6144$). Con un binario más rápido en el cálculo, la ventaja de un nodo es mayor.
