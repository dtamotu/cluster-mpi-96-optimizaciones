...
\begin{idea}
\textbf{El caso que motivó el estudio: 15 minutos por Wi-Fi.} A las 00:58:05 del 24 de septiembre, multiplicar matrices de $3072\times3072$ con \textbf{8 procesos} tardó \textbf{906.351 s}. El log registró un máximo de \textbf{889.744 s en \archivo{MPI_Bcast(B)}} y solo \textbf{1.735 s de cálculo}. El cociente $889.744/906.351=98.17\%$ muestra dónde se concentró el tiempo registrado; los máximos por fase pueden corresponder a rangos distintos y no se suman. En esta ejecución no se midieron MB transmitidos.
\end{idea}
...
Todos los tiempos son segundos medidos dentro del programa con \archivo{MPI_Wtime}. Se usa punto decimal. Los tamaños de tráfico están en \textbf{MB decimales}, donde $1\,\mathrm{MB}=10^6$ bytes. Los gráficos de las dos baterías usan escala lineal y muestran los valores; algunas figuras separan las redes con escalas verticales diferentes para hacer visibles sus resultados. Las figuras de la campaña de optimización usan escala logarítmica, indicada en su eje, porque comparan tiempos que difieren en dos órdenes de magnitud. Cuando hay repeticiones se informa la \textbf{mediana} y el rango mínimo--máximo, no la media ni un intervalo de confianza.
...
En la primera batería se cotejaron las 50 líneas de resultado con sus CSV. Sus 30 ejecuciones de matrices presentan las mismas huellas para cada tamaño, a la precisión impresa. En la segunda se cotejaron las 75 líneas y sus huellas. Son verificaciones de consistencia, no una demostración de igualdad elemento a elemento. Los scripts de medición extraen el tiempo; la revisión de huellas se hizo al generar este informe y se repite en la auditoría de la sección~\ref{sec:auditoria}.
...
\input{tablas/trapecio_100000000}
...
\subsection{Problema grande: \texorpdfstring{$n=10^{11}$}{n = 100000000000}}\label{sec:trap-grande}
\input{tablas/trapecio_100000000000}
...
\input{tablas/madrugada_original}
...
La campaña de optimización aporta evidencia medida, aunque indirecta. Con $N=3072$, la variante 1D global transmitió 358.0 MB agregados con $P=64$ y 755.5 MB con $P=96$, mientras la difusión jerárquica explícita con $P=96$ transmitió 358.9 MB. El patrón por defecto, por tanto, sí cambia con $P$ y con 96 procesos mueve unas 2.1 veces más datos que una difusión por nodo. En la difusión aislada con 16 procesos, la colectiva por defecto y la selección forzada \texttt{scatter\_allgather} dieron el mismo TX (1006.5 MB por Wi-Fi), lo que es compatible con la tabla de decisión citada. Sigue sin registrarse qué algoritmo se eligió en cada ejecución.
